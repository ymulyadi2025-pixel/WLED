...
class MizumaSmartSystem : public Usermod {
  private:
    ...
    // Reminder servis: simpan waktu terakhir ganti (epoch detik) + interval (hari)
    struct ReminderItem {
    ...
    // Mapping preset LED (10 slot, sesuai roadmap: mode x sisi)
  ...
  public:
    void setup() override {
      Serial.println(F("[Mizuma] Usermod utama siap"));
    }

    void loop() override {
      // Logic reminder-check & lainnya ditambah di fase berikutnya
    }
