...
// =====================================================================================================================================
// BLOK 5 — LED (REVISI V4: Fixing Star Palettes, Precision WLED FastLED Color Palette & Auto-Retry PalX)
// =====================================================================================================================================
...
.info-row3{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.5fr) minmax(0,1fr);gap:8px;align-items:center;padding:4px 12px 10px;background:var(--bg);border-bottom:1px solid var(--bd);}
...
.pane-head{flex-shrink:0;background:var(--bg);padding:8px 12px;border-bottom:1px solid var(--bd);display:flex;flex-direction:column;gap:6px;}
.pane-scroll{flex:1;min-height:0;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:8px 10px;}
.pane-foot{flex-shrink:0;background:var(--s1);border-top:1px solid var(--bd);padding:10px 12px;max-height:45%;overflow-y:auto;}
.mode-row{font-size:10.5px;color:var(--tx2);}
.mode-row b{color:var(--tx);}
.search-box{width:100%;padding:7px 10px;background:var(--s1);border:1px solid var(--bd);border-radius:10px;color:var(--tx);font-size:11px;}
...
.vsl{position:relative;width:24px;height:120px;display:flex;align-items:center;justify-content:center;}
.vsl input{position:absolute;top:50%;left:50%;width:120px;height:20px;transform:translate(-50%,-50%) rotate(-90deg);}
.vsl-l{position:absolute;bottom:-14px;left:50%;transform:translateX(-50%);font-size:9px;color:var(--tx2);font-weight:800;}
...
.crow-btn{position:relative;flex:1;max-width:85px;height:36px;border-radius:10px;border:1px solid var(--bd2);color:#fff;font-size:12px;font-weight:800;display:flex;align-items:center;justify-content:center;text-shadow:0 1px 2px rgba(0,0,0,.8);box-shadow:inset 0 0 0 1px rgba(255,255,255,.1);transition:all .15s var(--ease);}
.crow-btn.active{border-color:var(--ac);box-shadow:0 0 10px rgba(245,165,36,.35), inset 0 0 0 1px var(--ac);transform:scale(1.04);}
.crow-btn .slot-tag{font-size:7.5px;padding:1px 4px;border-radius:4px;background:rgba(0,0,0,.6);color:var(--ac);position:absolute;top:2px;right:3px;font-weight:800;}
.slot-legend{font-size:9px;color:var(--tx2);text-align:center;margin-top:6px;line-height:1.2;display:flex;gap:6px;justify-content:center;flex-wrap:wrap;}
...
.fx-item{background:var(--s1);border:1px solid var(--bd);border-radius:10px;padding:9px 7px;font-size:11px;color:var(--tx);cursor:pointer;}
...
.restricted-note{font-size:10.5px;color:var(--tx3);text-align:center;padding:8px 0 4px;}
.saved-row{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--bd);font-size:11.5px;}
...
.mbtns .btn-sm{flex:1;}
.btn-sm{padding:9px 14px;border-radius:8px;background:var(--s2);border:1px solid var(--bd2);font-size:12px;font-weight:700;color:var(--tx);cursor:pointer;}
.btn-sm.primary{background:var(--ac);color:#0a0a0a;border-color:var(--ac);}
input[type=range]{-webkit-appearance:none;appearance:none;background:transparent;height:16px;--p:50%;}
...
<div class="info-row3">
<div class="i3 left" id="ctxInfo">Kiri: -</div>
<div class="i3 mid">
...
<div class="i3 right" id="savedInfo">Kanan: -</div>
</div>
</div>
...
<div class="pane-scroll" id="scrollTemplate" style="display:none;">
<div class="palette-list" id="paletteGrid"></div>
</div>

<div class="pane-scroll" id="scrollCustom" style="display:none;">
<div class="custom-grid">
<div class="cg-wheel"><canvas id="colorWheel" width="160" height="160"></canvas></div>
<div class="cg-vsl">
...
<div class="slot-header">Target Slot Warna</div>
<div class="crow" id="colorRow"></div>
<div class="slot-legend">
<span><b>fx</b> = warna efek</span> &bull; 
<span><b>bg</b> = warna background</span> &bull; 
<span><b>3</b> = warna tambahan</span>
</div>
</div>
...
<div class="cg-wheel" style="max-width:160px;margin:0 auto;"><canvas id="wheelR" width="160" height="160"></canvas></div>
...
let wHue=0,wSat=1,wVal=1,liveEnabled=true;
...
const QUICK_COLORS=['FF0000','FFA500','FFD200','FFFFFF','000000','0000FF','00FF00'];
const FX_PAL_DEFAULTS={'Fire 2012':'Fire','Fireworks 1D':'Rainbow','Fireworks':'Rainbow','Noise 1':'Pastel','Noise':'Pastel','Pride':'Pride','Aurora':'Aurora','Pacifica':'Pacifica'};
function palIdByName(n){return palNamesRaw.findIndex(function(x){return x===n;});}
...
fetch('/json/state').then(function(r){return r.json();}).then(function(j){if(j.seg&&j.seg.length>0){segR={start:j.seg[0].start||0,stop:j.seg[0].stop||48};if(j.seg.length>1)segL={start:j.seg[1].start||48,stop:j.seg[1].stop||96};if(segL.start<segR.stop)segR.stop=segL.start;}}).catch(function(){});
let liveOk=false,mzWS=null,mzWSRetry=null,lastDraw=0;
function mzConnectLive(){if(mzWSRetry){clearTimeout(mzWSRetry);mzWSRetry=null;}try{mzWS=new WebSocket('ws://'+window.location.host+'/ws');}catch(e){mzWSRetry=setTimeout(mzConnectLive,3000);return;}mzWS.binaryType='arraybuffer';mzWS.onopen=function(){try{mzWS.send(JSON.stringify({lv:true}));}catch(e){}};mzWS.onmessage=function(evt){if(!liveEnabled)return;const now=performance.now();if(now-lastDraw<33)return;lastDraw=now;let flat=null;if(evt.data instanceof ArrayBuffer){flat=new Uint8Array(evt.data);}else if(typeof evt.data==='string'){try{const j=JSON.parse(evt.data);const L=j.leds;if(!L)return;if(Array.isArray(L)){if(Array.isArray(L[0])){flat=new Uint8Array(L.length*3);for(let i=0;i<L.length;i++){flat[i*3]=L[i][0];flat[i*3+1]=L[i][1];flat[i*3+2]=L[i][2];}}else{flat=new Uint8Array(L);}}}catch(e){return;}}if(!flat||flat.length<3)return;liveOk=true;drawLive(flat);};mzWS.onclose=function(){liveOk=false;mzWS=null;if(!document.hidden)mzWSRetry=setTimeout(mzConnectLive,2000);};mzWS.onerror=function(){try{mzWS.close();}catch(e){}};}
mzConnectLive();
window.addEventListener('beforeunload',function(){if(mzWS)try{mzWS.close();}catch(e){}});
function pollState(){if(!liveEnabled)return;fetch('/json/state').then(function(r){return r.json();}).then(function(j){const s=j.seg||[];const f=function(cv,seg){if(!seg)return;const c=(seg.col&&seg.col[0])?seg.col[0]:[20,20,20];const ctx=cv.getContext('2d');ctx.fillStyle='rgb('+c[0]+','+c[1]+','+c[2]+')';ctx.fillRect(0,0,48,1);};f(pvR,s[0]);f(pvL,s[1]||s[0]);}).catch(function(){});}
setInterval(function(){if(!liveOk)pollState();},1000);
document.getElementById('liveToggle').addEventListener('click',function(){liveEnabled=!liveEnabled;this.classList.toggle('on',liveEnabled);pvR.classList.toggle('off',!liveEnabled);pvL.classList.toggle('off',!liveEnabled);});

/* ===== COLOR MATH ===== */
function hsvToRgb(h,s,v){const c=v*s,x=c*(1-Math.abs((h/60)%2-1)),m=v-c;let r,g,b;if(h<60){r=c;g=x;b=0;}else if(h<120){r=x;g=c;b=0;}else if(h<180){r=0;g=c;b=x;}else if(h<240){r=0;g=x;b=c;}else if(h<300){r=x;g=0;b=c;}else{r=c;g=0;b=x;}return[Math.round((r+m)*255),Math.round((g+m)*255),Math.round((b+m)*255)];}
...
function clampHue(h,r){if(!r)return h;if(r[0]<=r[1])return h<r[0]?r[0]:(h>r[1]?r[1]:h);if(h>r[1]&&h<r[0])return (h-r[1])<=(r[0]-h)?r[1]:r[0];return h;}
const wheelImgs={};
...
function paintWheel(cv,hue,sat){const e=wheelImgs[cv.id];if(!e)return;const ctx=cv.getContext('2d');ctx.putImageData(e.img,0,0);const wr=cv.width/2,th=(hue-180)*Math.PI/180,r=sat*(wr-2),x=wr+Math.cos(th)*r,y=wr+Math.sin(th)*r;ctx.beginPath();ctx.arc(x,y,5,0,6.2832);ctx.strokeStyle='#fff';ctx.lineWidth=2;ctx.stroke();ctx.beginPath();ctx.arc(x,y,7,0,6.2832);ctx.strokeStyle='rgba(0,0,0,.65)';ctx.lineWidth=1.5;ctx.stroke();}
const wheel=document.getElementById('colorWheel');buildWheelImg(wheel,null);paintWheel(wheel,wHue,wSat);
function wheelPick(e){const rect=wheel.getBoundingClientRect(),wr=wheel.width/2,x=(e.clientX-rect.left)*(wheel.width/rect.width),y=(e.clientY-rect.top)*(wheel.height/rect.height),dx=x-wr,dy=y-wr,d=Math.sqrt(dx*dx+dy*dy);if(d>wr)return;wHue=Math.atan2(dy,dx)*180/Math.PI+180;wSat=Math.min(1,d/wr);paintWheel(wheel,wHue,wSat);const rgb=hsvToRgb(wHue,wSat,wVal);setColor(rgb[0],rgb[1],rgb[2],false);}
let dragging=false;
...
document.getElementById('valSlider').addEventListener('input',function(e){wVal=(+e.target.value)/100;const rgb=hsvToRgb(wHue,wSat,wVal);setColor(rgb[0],rgb[1],rgb[2],false);});
function kelvinToRgb(k){k/=100;let r,g,b;if(k<=66){r=255;g=99.47*Math.log(k)-161.12;}else{r=329.7*Math.pow(k-60,-0.133);g=288.12*Math.pow(k-60,-0.0755);}if(k>=66)b=255;else if(k<=19)b=0;else b=138.52*Math.log(k-10)-305.04;const cl=function(v){return Math.max(0,Math.min(255,Math.round(v)));};return[cl(r),cl(g),cl(b)];}
document.getElementById('kelvinSlider').addEventListener('input',function(e){const rgb=kelvinToRgb(+e.target.value);setColor(rgb[0],rgb[1],rgb[2],false);});
...
function sendColor(r,g,b,silent){if(isRestrictedTab()){colorTarget=0;cur.pal=0;segColors[0]=[r,g,b];segColors[1]=[0,0,0];segColors[2]=[0,0,0];}else{segColors[colorTarget]=[r,g,b];}const cols=[segColors[0]||[255,255,255],segColors[1]||[0,0,0],segColors[2]||[0,0,0]];const segs=segIds().map(function(id){return{id:id,col:cols,pal:isRestrictedTab()?0:cur.pal};});post({seg:segs});cur.col=[r,g,b];if(!silent)markDirty();renderColorRow();updateSideInfo();}
function sendPalette(i){if(isRestrictedTab())return;const segs=segIds().map(function(id){return{id:id,pal:i};});post({seg:segs});cur.pal=i;markDirty();renderColorRow();updateSideInfo();}
function sendEffect(i){if(isRestrictedTab())normalizeRestrictedState(false);cur.fx=i;cur.params=paramsFor(i);let palToSend=cur.pal;if(!isRestrictedTab()){const nm=allFx[i]||'';if(FX_PAL_DEFAULTS[nm]){const pi=palIdByName(FX_PAL_DEFAULTS[nm]);if(pi>=0){cur.pal=pi;cur.palName=FX_PAL_DEFAULTS[nm];palToSend=pi;}}}const cols=[segColors[0]||[255,255,255],isRestrictedTab()?[0,0,0]:(segColors[1]||[0,0,0]),isRestrictedTab()?[0,0,0]:(segColors[2]||[0,0,0])];const segs=segIds().map(function(id){const o={id:id,fx:i,sx:cur.params.sx,ix:cur.params.ix,c1:cur.params.c1,c2:cur.params.c2,c3:cur.params.c3,o1:cur.params.o1,o2:cur.params.o2,o3:cur.params.o3};if(isRestrictedTab()){o.pal=0;o.col=cols;}else{o.pal=palToSend;}return o;});post({seg:segs});if(isRestrictedTab())cur.pal=0;markDirty();renderColorRow();updateSideInfo();}
...
const sendBriD=debounce(function(v){post({bri:v});fetch('/mizuma/bri?v='+v,{method:'POST'}).catch(function(){});},80);

/* ===== SIDE INFO ===== */
function updateSideInfo(){fetch('/json/state').then(function(r){return r.json();}).then(function(j){if(!j.seg)return;const nm=function(s){if(!s)return '-';if(s.on===false)return 'Off';return (allFx[s.fx]||'Efek')+' - '+(palNamesRaw[s.pal]||'Palette');};document.getElementById('ctxInfo').textContent=nm(j.seg[1]||j.seg[0]);document.getElementById('savedInfo').textContent=nm(j.seg[0]);}).catch(function(){});}

/* ===== EFEK / BG / CS HELPER & STAR PALETTES ===== */
function isPSFx(){const n=allFx[cur.fx]||'';return n.indexOf('PS ')===0;}
function gstr(a){return 'rgb('+(a[0]||0)+','+(a[1]||0)+','+(a[2]||0)+')';}

function starGrad(name){
  if(!name) return null;
  const c0=segColors[0]||[255,255,255],c1=segColors[1]||c0,c2=segColors[2]||c1;
  const nm=name.toLowerCase();
  if(nm.includes('color 1') && !nm.includes('1&2')) return gstr(c0);
  if(nm.includes('1&2') || nm.includes('1 & 2')) return 'linear-gradient(90deg, '+gstr(c0)+', '+gstr(c1)+')';
  if(nm.includes('gradient')) return 'linear-gradient(90deg, '+gstr(c2)+', '+gstr(c1)+', '+gstr(c0)+')';
  if(nm.includes('colors only') || nm.includes('only')) return 'linear-gradient(90deg, '+gstr(c0)+' 0 33%, '+gstr(c1)+' 33% 66%, '+gstr(c2)+' 66% 100%)';
  return 'linear-gradient(90deg, '+gstr(c0)+', '+gstr(c1)+', '+gstr(c2)+')';
}
...
function updateStarStrips(){
  document.querySelectorAll('.pal-row').forEach(function(row){
    ...
    if(name.charAt(0)==='*'){
      ...
      if(strip) strip.style.background = starGrad(name);
    }
  });
}
...
function renderColorRow(){
  ...
  const mk=function(label,idx,sub){
    const b=document.createElement('button');
    b.className='crow-btn'+(colorTarget===idx?' active':'');
    b.style.background=gstr(segColors[idx]||[128,128,128]);
    b.innerHTML='<span>'+label+'</span>'+(colorTarget===idx?'<span class="slot-tag">AKTIF</span>':'');
    b.title=sub;
    b.addEventListener('click',function(){colorTarget=idx;renderColorRow();});
    box.appendChild(b);
  };
  if(!(ps&&!star))mk('fx',0,'Warna Efek');
  mk('bg',1,'Warna Background');
  if(star&&slotCount>=3)mk('3',2,'Warna Tambahan');
  updateStarStrips();
}

(function(){const box=document.getElementById('quickColors');QUICK_COLORS.forEach(function(h){const d=document.createElement('div');d.className='qc';d.style.background='#'+h;d.addEventListener('click',function(){const n=parseInt(h,16);setColor((n>>16)&255,(n>>8)&255,n&255,false);});box.appendChild(d);});const r=document.createElement('div');r.className='qc';r.style.background='conic-gradient(#f00,#ff0,#0f0,#0ff,#00f,#f0f,#f00)';r.style.position='relative';r.innerHTML='<span style="position:absolute;inset:4px;border-radius:50%;background:var(--bg);color:var(--tx);display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:800;">R</span>';r.addEventListener('click',function(){setColor(Math.random()*256|0,Math.random()*256|0,Math.random()*256|0,false);});box.appendChild(r);})();
...
/* ===== PALETTES (PRESISI FASTLED WLED ORI + RETRY AUTO-FETCH) ===== */
const paletteGrid=document.getElementById('paletteGrid'),customPalGrid=document.getElementById('customPalGrid');
let palXGrad={};

// Tabel Warna Gradien Bawaan FastLED WLED Original
const KNOWN_PAL_GRADS = {
  'default': 'linear-gradient(90deg, #f00, #ff0, #0f0, #0ff, #00f, #f0f, #f00)',
  'aurora': 'linear-gradient(90deg, rgb(0,255,170), rgb(0,136,255), rgb(119,0,255), rgb(0,255,170))',
  'aurora 2': 'linear-gradient(90deg, rgb(0,51,170), rgb(0,255,136), rgb(0,255,255), rgb(0,51,170))',
  'autumn': 'linear-gradient(90deg, rgb(35,0,0), rgb(180,40,0), rgb(255,120,0), rgb(255,200,0))',
  'beach': 'linear-gradient(90deg, rgb(1,1,1), rgb(200,150,50), rgb(16,128,208), rgb(255,255,255))',
  'beech': 'linear-gradient(90deg, rgb(34,17,0), rgb(136,85,34), rgb(220,160,80), rgb(255,255,255))',
  ...
  'c9': 'linear-gradient(90deg, rgb(255,0,0), rgb(0,255,0), rgb(0,0,255), rgb(255,136,0), rgb(255,0,0))',
  'c9 2': 'linear-gradient(90deg, rgb(255,0,0), rgb(0,255,0), rgb(0,0,255), rgb(255,255,255), rgb(255,0,0))',
  'c9 new': 'linear-gradient(90deg, rgb(255,0,0), rgb(0,255,0), rgb(0,0,255), rgb(255,255,0), rgb(255,0,0))',
  'candy': 'linear-gradient(90deg, rgb(255,0,0), rgb(255,255,255), rgb(255,0,0), rgb(255,255,255))',
  'candy2': 'linear-gradient(90deg, rgb(255,0,0), rgb(0,0,0), rgb(255,255,255), rgb(0,0,0), rgb(255,0,0))',
  'cloud': 'linear-gradient(90deg, rgb(17,34,68), rgb(102,136,170), rgb(255,255,255), rgb(136,170,204))',
  'cyane': 'linear-gradient(90deg, rgb(0,15,30), rgb(0,120,180), rgb(0,240,255))',
  'forest': 'linear-gradient(90deg, rgb(0,34,0), rgb(0,170,34), rgb(136,255,0), rgb(255,255,136))',
  'lava': 'linear-gradient(90deg, rgb(0,0,0), rgb(255,0,0), rgb(255,170,0), rgb(255,255,255))',
  'ocean': 'linear-gradient(90deg, rgb(0,5,17), rgb(0,68,170), rgb(0,204,255), rgb(255,255,255))',
  'party': 'linear-gradient(90deg, rgb(85,0,255), rgb(255,0,170), rgb(255,0,0), rgb(255,255,0), rgb(0,255,0))',
  'rainbow': 'linear-gradient(90deg, #ff0000, #ff8800, #ffff00, #00ff00, #0000ff, #8800ff)',
  'sunset': 'linear-gradient(90deg, rgb(10,0,30), rgb(120,0,60), rgb(240,80,0), rgb(255,220,0))'
};

function getPalGrad(id, name){
  if(!name) name = palNamesRaw[id] || '';
  if(name.charAt(0)==='*') return starGrad(name) || 'linear-gradient(90deg, #f00, #ff0)';
...
/* FETCH ALL PALETTES TERMASUK PALETTE KHUSUS (*) */
fetch('/json/pal').then(function(r){return r.json();}).then(function(names){
  ...
  arr.sort(function(a,b){
    if(a.n.charAt(0)==='*') return -1;
    if(b.n.charAt(0)==='*') return 1;
...
/* ===== MODAL ===== */
function showModal(action){pendingAction=action;document.getElementById('modalCtx').textContent=tabLabel()+' \u2014 '+sideLabel();document.getElementById('saveModal').style.display='flex';}
...
if(activeSide==='kanan'){post({seg:[{id:SEG_L,on:false}]});}
else if(activeSide==='kiri'){post({seg:[{id:SEG_K,on:false}]});}
updateCtx();updateSideInfo();}).catch(function(){});}

/* ===== NAVIGATION ===== */
function setSub(k){activeSub=k;const r=isRestrictedTab();
document.getElementById('headEfek').style.display=k==='efek'?'flex':'none';
document.getElementById('headWarna').style.display=((k==='template'||k==='custom')&&!r)?'flex':'none';
...
if(r&&(k==='template'||k==='custom')){buildRestricted();normalizeRestrictedState(false);}
if(k==='simpan')renderSavedList();
updateModeAktif();}
function switchTab(t){activeTab=t;localStorage.setItem('mzActiveTab',t);document.querySelectorAll('#tabbar button').forEach(function(b){b.classList.toggle('active',b.dataset.tab===t);});setSub(activeSub);if(!allFx.length){loadFxData(function(ok){buildEffects();applySaved(t);});}else{buildEffects();applySaved(t);}updateSideInfo();}
function commitSide(side){activeSide=side;document.querySelectorAll('#sideRow .side-btn').forEach(function(b){b.classList.toggle('active',b.dataset.side===side);});applySaved(activeTab);}
...
document.getElementById('sideRow').addEventListener('click',function(e){const btn=e.target.closest('button');if(!btn||!btn.dataset.side)return;const side=btn.dataset.side;if(side===activeSide)return;if(dirty){showModal(function(){commitSide(side);});}else{commitSide(side);}});
...
setSub(activeSub);renderSavedList();
...
StaticJsonDocument<1024> doc;JsonObject root=doc.to<JsonObject>();
root["on"]=true;if(bri==0)root["bri"]=180;
JsonArray seg=root.createNestedArray("seg");
addSegmentFromSlot(seg,segId,slotIdx);
deserializeState(root);}
void applyTwoSlots(uint8_t rightSlot,uint8_t leftSlot){
StaticJsonDocument<2048> doc;JsonObject root=doc.to<JsonObject>();
root["on"]=true;root["transition"]=0;if(bri==0)root["bri"]=180;
JsonArray seg=root.createNestedArray("seg");
addSegmentFromSlot(seg,0,rightSlot);
addSegmentFromSlot(seg,1,leftSlot);
deserializeState(root);}
int modeBase(const String &mode){
if(mode=="welcoming")return 0;if(mode=="riding")return 2;if(mode=="sein")return 4;
if(mode=="rem")return 6;if(mode=="hazard")return 8;return -1;}
bool applyModeSide(const String &mode,const String &side){
int base=modeBase(mode);if(base<0)return false;
if(side=="both"){applyTwoSlots(base,base+1);return true;}
if(side=="kanan"){applySingleSlot(base);return true;}
if(side=="kiri"){applySingleSlot(base+1);return true;}
return false;}
...
PresetSlot p;p.valid=true;
p.fx=argU8(req,"fx",0);p.pal=argU8(req,"pal",0);
p.r=argU8(req,"r",255);p.g=argU8(req,"g",255);p.b=argU8(req,"b",255);
p.r1=argU8(req,"r1",0);p.g1=argU8(req,"g1",0);p.b1=argU8(req,"b1",0);
p.r2=argU8(req,"r2",0);p.g2=argU8(req,"g2",0);p.b2=argU8(req,"b2",0);
p.sx=argU8(req,"sx",128);p.ix=argU8(req,"ix",128);
p.c1=argU8(req,"c1",128);p.c2=argU8(req,"c2",128);p.c3=argU8(req,"c3",128);
p.o1=req->hasArg("o1")?(req->arg("o1").toInt()?1:0):0;
p.o2=req->hasArg("o2")?(req->arg("o2").toInt()?1:0):0;
p.o3=req->hasArg("o3")?(req->arg("o3").toInt()?1:0):0;
p.bri=argU8(req,"bri",180);if(p.bri==0)p.bri=180;
if(isRestrictedSlot(i)){p.pal=0;p.r1=0;p.g1=0;p.b1=0;p.r2=0;p.g2=0;p.b2=0;
if(p.r==0&&p.g==0&&p.b==0){if(i==6||i==7){p.r=255;p.g=0;p.b=0;}else{p.r=255;p.g=165;p.b=0;}}}
pslots[i]=p;
serializeConfigToFS();
req->send(200,"application/json","{\"ok\":true}");}

void handleApply(AsyncWebServerRequest *req){
if(!req->hasArg("mode")||!req->hasArg("side")){req->send(400,"application/json","{\"ok\":false}");return;}
bool ok=applyModeSide(req->arg("mode"),req->arg("side"));
req->send(200,"application/json",ok?"{\"ok\":true}":"{\"ok\":false}");}

/* FIX BUG 1: handler brightness permanen */
...
String html=FPSTR(pageTemplate);applyCommonReplacements(html,activeKey);return html;}
String renderPlaceholder(const char* title,const char* icon,const char* activeKey){
String html=FPSTR(MIZUMA_PLACEHOLDER_HTML);
html.replace("%PAGE_TITLE%",title);html.replace("%PAGE_ICON%",icon);
applyCommonReplacements(html,activeKey);return html;}
...
res->addHeader("Cache-Control","max-age=86400");req->send(res);});
...
res->addHeader("Cache-Control","max-age=86400");req->send(res);});
server.on("/led",HTTP_GET,[](AsyncWebServerRequest *req){req->send_P(200,"text/html",MIZUMA_LED_HTML);});
server.on("/app",HTTP_GET,[this](AsyncWebServerRequest *req){req->send(200,"text/html",renderPage(MIZUMA_HOME_HTML,"beranda"));});
server.on("/pengaturan",HTTP_GET,[this](AsyncWebServerRequest *req){req->send(200,"text/html",renderPage(MIZUMA_SETTINGS_HTML,"pengaturan"));});
server.on("/servis",HTTP_GET,[this](AsyncWebServerRequest *req){req->send(200,"text/html",renderPlaceholder("Servis","🛠","servis"));});
server.on("/keamanan",HTTP_GET,[this](AsyncWebServerRequest *req){req->send(200,"text/html",renderPlaceholder("Keamanan & GPS","🔒","keamanan"));});
server.on("/mizuma/frag/header",HTTP_GET,[](AsyncWebServerRequest *req){req->send_P(200,"text/html",MIZUMA_HEADER_HTML);});
server.on("/mizuma/frag/script",HTTP_GET,[](AsyncWebServerRequest *req){req->send_P(200,"text/javascript",MIZUMA_HEADER_SCRIPT);});
server.on("/mizuma/frag/nav",HTTP_GET,[](AsyncWebServerRequest *req){req->send_P(200,"text/html",MIZUMA_BOTTOMNAV_HTML);});
server.on("/mizuma/status",HTTP_GET,[](AsyncWebServerRequest *req){
bool apOn=(WiFi.softAPgetStationNum()>0);bool staOn=(WiFi.status()==WL_CONNECTED);
...
json+="\"}";req->send(200,"application/json",json);});
server.on("/mizuma/debug",HTTP_GET,[](AsyncWebServerRequest *req){
char buf[192];snprintf(buf,sizeof(buf),"{\"heap\":%u,\"minHeap\":%u,\"maxAlloc\":%u}",
(unsigned)ESP.getFreeHeap(),(unsigned)ESP.getMinFreeHeap(),(unsigned)ESP.getMaxAllocHeap());
req->send(200,"application/json",buf);});
server.on("/mizuma/preset",HTTP_GET,[this](AsyncWebServerRequest *req){handlePreset(req);});
server.on("/mizuma/preset",HTTP_POST,[this](AsyncWebServerRequest *req){handlePreset(req);});
server.on("/mizuma/apply",HTTP_GET,[this](AsyncWebServerRequest *req){handleApply(req);});
server.on("/mizuma/apply",HTTP_POST,[this](AsyncWebServerRequest *req){handleApply(req);});
server.on("/mizuma/bri",HTTP_GET,[this](AsyncWebServerRequest *req){handleBri(req);});
server.on("/mizuma/bri",HTTP_POST,[this](AsyncWebServerRequest *req){handleBri(req);});
server.on("/mizuma/presets",HTTP_GET,[this](AsyncWebServerRequest *req){
DynamicJsonDocument doc(4096);JsonObject root=doc.to<JsonObject>();
for(int i=0;i<10;i++){PresetSlot p=getEffectiveSlot(i);
JsonObject s=root.createNestedObject(slotKey(i));
s["valid"]=pslots[i].valid;s["fx"]=p.fx;s["pal"]=p.pal;
JsonArray c=s.createNestedArray("col");
JsonArray c0=c.createNestedArray();c0.add(p.r);c0.add(p.g);c0.add(p.b);
JsonArray c1=c.createNestedArray();c1.add(p.r1);c1.add(p.g1);c1.add(p.b1);
JsonArray c2=c.createNestedArray();c2.add(p.r2);c2.add(p.g2);c2.add(p.b2);
s["sx"]=p.sx;s["ix"]=p.ix;s["c1"]=p.c1;s["c2"]=p.c2;s["c3"]=p.c3;
s["o1"]=p.o1;s["o2"]=p.o2;s["o3"]=p.o3;s["bri"]=p.bri;}
String out;serializeJson(doc,out);req->send(200,"application/json",out);});}

void loop() override {
  if(!briRestored){briRestored=true;bri=savedBri;stateUpdated(CALL_MODE_DIRECT_CHANGE);}
  if(briSavePending&&millis()-briSaveT>3000){briSavePending=false;serializeConfigToFS();}
  if(bootDone)return;
  unsigned long m=millis();
  if(bootStage==0){applyModeSide("welcoming","both");bootStage=1;bootT=m;}
...
s["sx"]=pslots[i].sx;s["ix"]=pslots[i].ix;
s["c1"]=pslots[i].c1;s["c2"]=pslots[i].c2;s["c3"]=pslots[i].c3;
...
vehicleName=vehicle["name"]|"";vehicleBrand=vehicle["brand"]|"";vehicleYear=vehicle["year"]|0;vehiclePlate=vehicle["plate"]|"";
...
pslots[i].sx=s["sx"]|128;pslots[i].ix=s["ix"]|128;
pslots[i].c1=s["c1"]|128;pslots[i].c2=s["c2"]|128;pslots[i].c3=s["c3"]|128;
pslots[i].o1=s["o1"]|0;pslots[i].o2=s["o2"]|0;pslots[i].o3=s["o3"]|0;
pslots[i].bri=s["bri"]|180;}}
welcomeDur = 7000; // dikunci 7 detik (abaikan nilai lama yg tersimpan)
savedBri=top["bri"]|180;
return true;}
