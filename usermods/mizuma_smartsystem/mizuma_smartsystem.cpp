...
// =====================================================================================================================================
// BLOK 5 — LED (REVISI TOTAL: fokus sisi, info L/R, slider vertikal V/K, legend, palette asli, tanpa hardcode)
// =====================================================================================================================================
...
.quick-colors{display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin-bottom:10px;}
...
.crow-legend{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;font-size:9.5px;color:var(--tx3);margin-top:8px;}
...
<div class="info-row3">
<div class="i3 left" id="infoKiri">Kiri: -</div>
<div class="i3 mid">
...
<div class="i3 right" id="infoKanan">Kanan: -</div>
</div>
</div>
...
<div class="vsl"><input type="range" class="kelvin" min="2000" max="10000" step="50" value="6500" id="kelvinSlider"><span class="vsl-l">K</span></div>
</div>
</div>
...
<div class="pane-foot" id="footEfek"><div id="fxParams"></div><div class="foot-hint" id="footHint" style="display:none;">Efek ini tidak memiliki parameter tambahan.</div></div>
</div>
</div>
...
/* TANPA hardcode pairing: kirim fx saja, lalu baca balik palette dari device */
function sendEffect(i){if(isRestrictedTab())normalizeRestrictedState(false);cur.fx=i;cur.params=paramsFor(i);const cols=[segColors[0]||[255,255,255],isRestrictedTab()?[0,0,0]:(segColors[1]||[0,0,0]),isRestrictedTab()?[0,0,0]:(segColors[2]||[0,0,0])];const segs=segIds().map(function(id){const o={id:id,fx:i,sx:cur.params.sx,ix:cur.params.ix,c1:cur.params.c1,c2:cur.params.c2,c3:cur.params.c3,o1:cur.params.o1,o2:cur.params.o2,o3:cur.params.o3};if(isRestrictedTab()){o.pal=0;o.col=cols;}return o;});post({seg:segs});if(isRestrictedTab())cur.pal=0;markDirty();renderColorRow();setTimeout(syncPalFromDevice,300);updateSideInfo();}
function syncPalFromDevice(){if(isRestrictedTab())return;fetch('/json/state').then(function(r){return r.json();}).then(function(j){const s=(j.seg&&j.seg[0])?j.seg[0]:null;if(!s||s.pal==null)return;if(s.pal!==cur.pal){cur.pal=s.pal;cur.palName=palNamesRaw[s.pal]||'';renderColorRow();updateModeAktif();document.querySelectorAll('.pal-row').forEach(function(el){el.classList.toggle('active',el.dataset.palname===cur.palName);});}}).catch(function(){});}
...
/* ===== INFO KIRI/KANAN (read-back) ===== */
function updateSideInfo(){fetch('/json/state').then(function(r){return r.json();}).then(function(j){if(!j.seg)return;const nm=function(s){if(!s)return '-';if(s.on===false)return 'Off';return (allFx[s.fx]||'Efek')+' - '+(palNamesRaw[s.pal]||'Palette');};document.getElementById('infoKiri').textContent=nm(j.seg[1]||j.seg[0]);document.getElementById('infoKanan').textContent=nm(j.seg[0]);}).catch(function(){});}
/* ===== FX / BG / CS ===== */
...
/* ===== PALETTES (thumbnail ASLI, tanpa tebakan) ===== */
const paletteGrid=document.getElementById('paletteGrid'),customPalGrid=document.getElementById('customPalGrid');
let palXGrad={},palSamples={};
try{palSamples=JSON.parse(localStorage.getItem('mzPalSamples')||'{}');}catch(e){palSamples={};}
let sampling=false;
...
function samplePalettes(){if(sampling)return;sampling=true;const gradFx=(lookupExact('Palette')>=0)?lookupExact('Palette'):((lookupExact('Gradient')>=0)?lookupExact('Gradient'):-1);if(gradFx<0){sampling=false;return;}fetch('/json/state').then(function(r){return r.json();}).then(function(orig){const o0=(orig.seg&&orig.seg[0])?orig.seg[0]:{};const list=palList.slice();let idx=0;function step(){if(idx>=list.length){post({seg:[{id:0,fx:o0.fx||0,pal:o0.pal||0}]});if(orig.bri!=null)post({bri:orig.bri});localStorage.setItem('mzPalSamples',JSON.stringify(palSamples));sampling=false;renderPaletteRows(document.getElementById('searchBox').value.toLowerCase());return;}const e=list[idx];if(palSamples[e.i]||palXGrad[e.i]){idx++;step();return;}post({seg:[{id:0,fx:gradFx,pal:e.i}],bri:1});setTimeout(function(){fetch('/json/live').then(function(r){return r.json();}).then(function(lv){const px=lv.leds||[];const stops=[];const st=Math.max(1,Math.floor(px.length/5));for(let i=0;i<px.length;i+=st){const c=Array.isArray(px[i])?px[i]:[px[i]];stops.push('rgb('+(c[0]||0)+','+(c[1]||0)+','+(c[2]||0)+')');}if(stops.length>1)palSamples[e.i]='linear-gradient(90deg,'+stops.join(',')+')';idx++;step();}).catch(function(){idx++;step();});},120);}step();});}
...
fetch('/json/pal').then(function(r){return r.json();}).then(function(names){palNamesRaw=names;const arr=[];names.forEach(function(n,i){if(n!=='r')arr.push({n:n,i:i});});arr.sort(function(a,b){return a.n.localeCompare(b.n);});const di=arr.findIndex(function(e){return e.n==='Default'||e.i===0;});if(di>0){const d=arr.splice(di,1)[0];arr.unshift(d);}palList=arr;renderPaletteRows('');renderColorRow();loadPalX();updateSideInfo();}).catch(function(){});
...
function refreshSavedInfo(){const chip=document.getElementById('fxSavedName');if(!allFx.length)return;fetch('/mizuma/presets').then(function(r){return r.json();}).then(function(d){const first=activeSide==='kiri'?'Kiri':'Kanan';const st=d[activeTab+first];if(chip)chip.textContent=st&&st.valid?(allFx[st.fx]||'-'):'-';}).catch(function(){});}
/* ===== SAVE ===== */
...
/* FOKUS: matikan sisi yang tidak diedit */
if(activeSide==='kanan'){post({seg:[{id:SEG_L,on:false}]});}
else if(activeSide==='kiri'){post({seg:[{id:SEG_K,on:false}]});}
setSub(activeSub);refreshSavedInfo();updateSideInfo();}).catch(function(){});}
/* ===== NAVIGATION ===== */
...
if(k==='template'&&!r)samplePalettes();
if((k==='template'||k==='custom')&&r){buildRestricted();normalizeRestrictedState(false);}
if(k==='simpan')renderSavedList();
updateModeAktif();}
function switchTab(t){activeTab=t;localStorage.setItem('mzActiveTab',t);document.querySelectorAll('#tabbar button').forEach(function(b){b.classList.toggle('active',b.dataset.tab===t);});if(!allFx.length){loadFxData(function(ok){buildEffects();applySaved(t);});}else{buildEffects();applySaved(t);}}
