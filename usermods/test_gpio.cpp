...
class TestGpioUsermod : public Usermod {
  ...
  private:
    // Definisi pin sesuai mapping GPIO Mizuma SmartSystem
    static const uint8_t PIN_BUTTON      = 6;  // Tombol reset/AP fallback
    static const uint8_t PIN_RELAY_TEST  = 7;  // Relay anti-thief
    static const uint8_t PIN_STATUS_LED  = 8;  // LED status onboard
    static const uint8_t PIN_BATTERY_ADC = 0;  // Sensor tegangan aki

    unsigned long lastCheck = 0;
    const unsigned long checkInterval = 1000; // cek tiap 1 detik
  ...
  public:
    ...
    //  in setup()
    //   #ifdef USERMOD_EXAMPLE
    //   UM = (TestGpioUsermod*) UsermodManager::lookup(USERMOD_ID_EXAMPLE);
    //   #endif
     ...
     */
    void loop() override {
      // WAJIB non-blocking, JANGAN pakai delay() di sini
      if (millis() - lastCheck < checkInterval) return;
      lastCheck = millis();

      int buttonState = digitalRead(PIN_BUTTON);   // LOW = ditekan
    ...
    void addToJsonInfo(JsonObject& root) override
    {
      ...
      //this code adds "u":{"TestGpioUsermod":[20," lux"]} to the info object
      //int reading = 20;
    ...
    void readFromJsonState(JsonObject& root) override
    {
      ...
      JsonObject usermod = root[FPSTR(_name)];
      if (!usermod.isNull()) {
        // expect JSON usermod data in usermod name object: {"TestGpioUsermod:{"user0":10}"}
     ...
     * The configComplete variable is true only if the "TestGpioUsermod" object and all values are present.  If any values are missing, WLED will know to call addToConfig() to save them
     * 
...
const char TestGpioUsermod::_name[]    PROGMEM = "TestGpioUsermod";
const char TestGpioUsermod::_enabled[] PROGMEM = "enabled";


// implementation of non-inline member methods

void TestGpioUsermod::publishMqtt(const char* state, bool retain)
{
#ifndef WLED_DISABLE_MQTT
